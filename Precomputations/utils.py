...
def symmetrize_matrix(A):
    negative = np.tril(A[1:])
    positive = np.triu(A[1:])
    # Extract both directions
    pos_diagonals = positive.diagonal(axis1=1, axis2=2)
    positive = positive + np.transpose(positive, axes=(0,2,1))
    neg_diagonals = negative.diagonal(axis1=1, axis2=2)
    negative = negative + np.transpose(negative, axes=(0,2,1))
    # Extract diagonal (TODO: Remove for-loop)
    for i in range(len(A)-1):
        positive[i] = positive[i]-np.diag(pos_diagonals[i])
        negative[i] = negative[i]-np.diag(neg_diagonals[i])
    A = np.append(negative, [A[0]], axis=0)
    A = np.append(A, positive, axis=0)
    return A
